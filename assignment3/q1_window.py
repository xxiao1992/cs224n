...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        sentence = [start for i in range(window_size)] + sentence + [end for i in range(window_size)]

        for i in range(sen_len):
            cur_window = sentence[i:(i + (window_size * 2) + 1)]
            flatten = [item for sublist in cur_window for item in sublist]
            windowed_data.append((flatten, labels[i]))
        ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        shape_W = (self.config.n_window_features * self.config.embed_size, self.config.hidden_size)
